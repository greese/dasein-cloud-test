...
public class IpAddressTestCase extends BaseTestCase {
    ...
    public void setUp() throws CloudException, InternalException, InstantiationException, IllegalAccessException, IOException {
        ...
        for( String test : NEEDS_VLANS ) {
            if( getName().equals(test) ) {
                ...
                v = findTestVLAN(provider, networkServices == null ? null : networkServices.getVlanSupport(), true, true);
                if( v != null ) {
    ...
    public void testUnassignAddress() throws CloudException, InternalException {
        ...
        if( testAddress == null ) {
            ...
            if( !iok ) {
                if( !vok ) {
                    Assert.fail("System timed out before IP de-assignment reflected in both VM ('" + vm.getProviderAssignedIpAddressId() + "') and IP address ('" + address.getServerId() + "')");
                }
                else {
                    Assert.fail("System timed out before IP de-assignment reflected in IP address: '" + address.getServerId() + "'");
                }
            }
            else if( !vok ) {
                Assert.fail("System timed out before IP de-assignment reflected in VM: '" + vm.getProviderAssignedIpAddressId() + "'");
            }
        }
    }
